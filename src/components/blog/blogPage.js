      ...
      {cards && cards.map((item, index) => (
        <div key={index} className="sm:flex items-start gap-5 pb-12 lg:pb-16">
          <div className="lg:w-3/5 pb-5 lg:pb-0">
            <a href={item.href} aria-label="blog image">
              <img className="w-full md:w-96 h-auto rounded-3xl " src={item.image} alt={item.href} />
            </a>
          </div>
          <div className="w-full">
            <a >
              <h2 className='text-2xl md:text-4xl heading-4'>{item.heading}</h2>
            </a>
            <div className="pt-4">
              <a href={item.href} aria-label="author image" className="flex items-center gap-4">
